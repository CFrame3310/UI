local library = {
	...
	Default_Theme = {
        ...
        accentcolor = Color3.fromRGB(80, 245, 245),
        accentcolor2 = Color3.fromRGB(0, 151, 156),
        outlinecolor = Color3.fromRGB(60, 60, 60),
        outlinecolor2 = Color3.fromRGB(0, 0, 0),
        sectorcolor = Color3.fromRGB(30, 30, 30),
        toptextcolor = Color3.fromRGB(80, 245, 245),
        topheight = 48,
	...
	function library:CreateWindow(name, hidebutton)
		local window = {}
		window.name = name or ""
		window.size = UDim2.fromOffset(_G.Width, _G.Height)
		...
		function window:CreateTab(name)
			...
			updateevent.Event:Connect(function(theme)
				local size = textservice:GetTextSize(tab.name, theme.fontsize, Enum.Font.Code, Vector2.new(200, 300))
			...
			function tab:CreateSector(name, side)
				...
				updateevent.Event:Connect(function(theme)
					local size = textservice:GetTextSize(sector.name, 15, Enum.Font.Code, Vector2.new(2000, 2000))
					sector.Label.Size = UDim2.fromOffset(math.clamp(textservice:GetTextSize(sector.name, 15, Enum.Font.Code, Vector2.new(200, 300)).X + 13, 0, sector.Main.Size.X.Offset), size.Y)
				...
				function sector:AddToggle(text, default, callback, flag)
					...
					function toggle:AddDropdown(items, default, multichoice, callback, flag)
						...
						function dropdown:Clear()
							for i, v in pairs(dropdown.items) do
								...
								dropdown.ItemsFrame:FindFirstChild(v):Remove()
							end
					...
					function toggle:AddSlider(min, default, max, decimals, callback, flag)
						...
						updateevent.Event:Connect(function(theme)
							slider.InputLabel.Font = Enum.Font.Code
				...
				function sector:AddTextbox(text, default, callback, flag)
					...
					updateevent.Event:Connect(function(theme)
						textbox.Label.Font = Enum.Font.Code
					end)
					textbox.Holder = Instance.new("Frame", sector.Items)
					textbox.Holder.Name = "holder"
					textbox.Holder.ZIndex = 5
				...
				function sector:AddDropdown(text, items, default, multichoice, callback, flag)
					...
					function dropdown:Clear()
						for i, v in pairs(dropdown.items) do
							dropdown.ItemsFrame:FindFirstChild(v):Destroy()
							table.remove(dropdown.items, i)
						end
						--[[ dropdown.ItemsFrame.Size = UDim2.fromOffset(dropdown.Main.Size.X.Offset, math.clamp(#dropdown.items * item.AbsoluteSize.Y, 20, 156) + 4)
						dropdown.ItemsFrame.CanvasSize = UDim2.fromOffset(dropdown.ItemsFrame.AbsoluteSize.X, (#dropdown.items * item.AbsoluteSize.Y) + 4)
				...
				function sector:AddSeperator(text)
					...
					updateevent.Event:Connect(function(theme)
						seperator.label.Font = Enum.Font.Code
					...
					updateevent.Event:Connect(function(theme)
						textSize = textservice:GetTextSize(seperator.text, theme.fontsize,  Enum.Font.Code, Vector2.new(2000, 2000))
			...
			function tab:CreateThemeSystem(name)
				local ThemeSystem = {}
				ThemeSystem.theme = library.theme
				ThemeSystem.colorpickers = {}
				ThemeSystem.mainfolder = name
				ThemeSystem.secondfolder = name..'/Themes'
				ThemeSystem.defaulttheme = ThemeSystem.secondfolder..'Default.json'
				ThemeSystem.themecustomize = tab:CreateSector("Theme Customize","Left")
				ThemeSystem.themeconfig = tab:CreateSector("Theme Config","Right")
				ThemeSystem.makefolder = function()
					if not isfolder(ThemeSystem.mainfolder) then makefolder(ThemeSystem.mainfolder) end
				...
				ThemeSystem.getfileslist = function()
					...
					for i,v in pairs(listfiles(ThemeSystem.secondfolder)) do
						...
						table.insert(file,filename[2])
					end
					return file
				end
				
				for i,v in pairs(ThemeSystem.theme) do
					if typeof(v) == 'Color3' then
						local name = string.upper(string.sub(tostring(i),1,1))..string.sub(tostring(i),2,#tostring(i))
						print(i,v)
						ThemeSystem.colorpickers[i] = ThemeSystem.themecustomize:AddColorpicker(name,v,function(t)
							ThemeSystem.theme[i] = t
							window:UpdateTheme(ThemeSystem.theme)
						end)
					end
				end
				
				ThemeSystem.themeconfig:AddTextbox("Theme Name",'',function(t)
					theme_name = t
				end)

				ThemeSystem.themelist = ThemeSystem.themeconfig:AddDropdown("Select Theme",ThemeSystem:getfileslist(),'',false,function(t)
					selected_theme = t
				end)

				ThemeSystem.save = function()
					ThemeSystem:makefolder()
					local theme_json = {}
					for i,v in pairs(Default_Theme) do
						if typeof(v) == 'Color3' then
							theme_json[i] = 'Color3/'..tostring(v)
						else
							theme_json[i] = v
						end
					end
					writefile(ThemeSystem.secondfolder..'/'..theme_name..'.json',game.HttpService:JSONEncode(theme_json))
					Theme_List:Clear()
					for i,v in pairs(getlistfiles()) do
						Theme_List:Add(v)
					end
				end

				ThemeSystem.load = function()
					if isfolder(ThemeSystem.mainfolder) and isfolder(ThemeSystem.secondfolder) then
						local theme_json = game.HttpService:JSONDecode(readfile(ThemeSystem.secondfolder..'/'..selected_theme))
						for i,v in pairs(theme_json) do
							if string.find(tostring(v),'Color3/') then
								local Color3_ = string.split(tostring(v),'Color3/')[2]
								local rgb = string.split(Color3_,',')
								local r,g,b = rgb[1],rgb[2],rgb[3]
								theme_json[i] = Color3.fromRGB(math.floor((r*255)+0.5),math.floor((g*255)+0.5),math.floor((b*255)+0.5))
								ThemeSystem.colorpickers[i]:Set(theme_json[i])
							end
						end
						theme_json.font = Enum.Font.Code
						window:UpdateTheme(theme_json)
					end
				end
				
				ThemeSystem.setasdefault = function()
					ThemeSystem:makefolder()
					local theme_json = {}
					for i,v in pairs(ThemeSystem.theme) do
						if typeof(v) == 'Color3' then
					...
					writefile(ThemeSystem.defaulttheme,game.HttpService:JSONEncode(theme_json))
				end

				ThemeSystem.LoadDefaultTheme = function()
					ThemeSystem:makefolder()
					local theme_json = {}
					if isfile(ThemeSystem.defaulttheme) then
						local theme_json = game.HttpService:JSONDecode(readfile(ThemeSystem.defaulttheme))
						for i,v in pairs(theme_json) do
							if string.find(tostring(v),'Color3/') then
								local Color3_ = string.split(tostring(v),'Color3/')[2]
								local rgb = string.split(Color3_,',')
				...
				ThemeSystem.themeconfig:AddButton("Save Theme",function(t)
					ThemeSystem:save()
				end)

				ThemeSystem.themeconfig:AddButton("Load Theme",function(t)
					ThemeSystem:load()
				end)

				ThemeSystem.themeconfig:AddButton("Set as Default",function(t)
					ThemeSystem.setasdefault()
				end)

				ThemeSystem.themeconfig:AddButton("Reset Theme",function(t)
					for i,v in pairs(ThemeSystem.colorpicker) do
						ThemeSystem.colorpicker[i]:Set(Default_Theme[i])
					end
					window:UpdateTheme(Default_Theme)
				end)
				return ThemeSystem
			end
